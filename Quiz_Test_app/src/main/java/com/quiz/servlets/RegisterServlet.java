    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                if (result > 0) {
                    showSuccessPage(out, "Registration successful! Welcome " + username + "!", "login.html", "Go to Login");
                } else {
            ...
            if (e.getMessage().contains("Duplicate entry")) {
                if (e.getMessage().contains("username")) {
                    ...
                    showErrorPage(out, "Email already exists! Please use a different email.", "register.html");
                } else {
                    showErrorPage(out, "Username or email already exists! Please choose different values.", "register.html");
                }
            } else {
                showErrorPage(out, "Database error occurred. Please try again later.", "register.html");
            }
        }
    }
